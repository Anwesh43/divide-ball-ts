...
class DBNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new DBNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawDBNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : DBNode {
        var curr : DBNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
